...
def IOUCalc(w1, h1, box2):
    ...
    print("w1: {} h1: {} box2: {}".format(w1.size(), h1.size(), box2.size()))
    # Intersection area
...
def getTargets(model, targets, pred):
    ...
    ___, bestIdx = torch.stack(iou, 0).max(0)  # best iou and anchor
    ...
    tws = torch.log(gws / anchors[bestIdx,0])
    ths = torch.log(ghs / anchors[bestIdx,1])
